...
const Home = () => {
  ...
  const handleEmailCopy = () => {
    navigator.clipboard
      .writeText(process.env.REACT_APP_EMAIL)
      .then(() => {
        emailBtnRef.current.innerText = "Copied Successfully!"
        setTimeout(() => {
          emailBtnRef.current.innerText = process.env.REACT_APP_EMAIL
        }, 2000)
      })
      .catch((err) => {
      ...
      <div id="about">
        ...
        <div className="bio">
          ...
          <div className="objective">
            I help businesses get more reach by creating designs and bringing
            them to life while you work on what's most important.
          </div>
        </div>
      </div>
        ...
        {projects["html"].map((project) => {
          return (
            <Fragment key={project.title}>
              ...
              >
                <span className="projects-list-type">
                  {project.type} {"//"}{" "}
                </span>
                {project.title}
